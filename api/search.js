...
export default async function handler(req) {
  ...
  try {
    ...
    if (!query) return new Response(JSON.stringify({ error: 'Sorgu boş' }), { status: 400 });

    const apiKey = process.env.ANTHROPIC_API_KEY;
    if (!apiKey) return new Response(JSON.stringify({ error: 'API key yok' }), { status: 500 });
    ...
    const prompt = `Kullanıcı "${query}" ürününü arıyor. SADECE JSON döndür, başka hiçbir şey yazma:
{
  ...
  "cons":["eksi 1","eksi 2"],
  "verdict":"AI tavsiyesi: hangi mağazadan alınmalı (2 cümle Türkçe)"
}
